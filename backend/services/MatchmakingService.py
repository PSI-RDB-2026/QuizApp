...
class MatchmakingService:
    ...
    ELO_WINDOW_STEP_SECONDS = int(os.getenv("MATCHMAKING_ELO_WINDOW_STEP_SECONDS", "15"))
    ...
    def _compute_elo_window(joined_at: datetime) -> int:
        ...
        increments = waited_seconds // max(1, MatchmakingService.ELO_WINDOW_STEP_SECONDS)
        return min(
            MatchmakingService.MAX_ELO_WINDOW,
            MatchmakingService.BASE_ELO_WINDOW + increments * MatchmakingService.ELO_WINDOW_STEP,
        )
        ...
        async with MatchmakingService._lock:
            ...
            for existing in MatchmakingService._queue:
                if existing.game_mode != game_mode:
                    continue
                existing_window = MatchmakingService._compute_elo_window(existing.joined_at)
            ...
            for index, entry in enumerate(MatchmakingService._queue, start=1):
                if entry.uid == uid:
                    ...
                    return {
                        ...
                        "elo_window": MatchmakingService._compute_elo_window(entry.joined_at),
                    }
            return {
                "in_queue": False,
                "queue_position": None,
